...
void sampleSoil(SharedState &s, bool verbose = false) {
    ...
    static bool isWatered = false; // flag to track if we already watered
    ...
    if ((millis() - s.lastSampleTime) > 100) {
        ...
        if (readIndex >= SAMPLE_COUNT) {
            ...
            if (decisionCount == 0) {
                ...
                // even prevents manually overwatering
               if (((averageVoltage < MAX_MOISTURE_THRESHOLD) || (millis() - s.pumpStartTime) > MAX_PUMP_TIME_MS) && !isWatered) {
...
void button(SharedState &s) {
  ...
  if ((millis() - lastEdgeMs) > DEBOUNCE_MS && r != lastStable) {
    ...
    if (lastStable == HIGH) {
      if (s.pumpRunning) setState(s, Action::DRY);
      else               setState(s, Action::WATER);
